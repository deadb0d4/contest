...
def run_tests(job, tests):
    ...
    for filename in tests:
        ...
        if res.returncode != 0:
            ...
            err, ann = diff_lines(
                tr["cout"], get_head(os.path.join(output_dir, filename), MAX_LENGTH)
            )
            if err:
                tr["status"] = "WA"
                tr["cout"] = ann
...
parser.add_argument("--reset", "-r", action="count", help="reset workspace", default=0)
parser.add_argument(
    "--add_test", "-t", action="count", help="add test cases", default=0
)
parser.add_argument("--specific_test", "-s", nargs="*", help="use only selected tests")
